...
var passwordCheckOld = function () 
{
    ...
    //if password is meeting these requirements or not user will be getting these messages
    stringMessage1 = "Is at least 8 characters long!";
    stringMessage2 = "Does not meet the character length requirement.";
    stringMessage3 = "Is not a known bad password!";
    stringMessage4 = "Is a known bad password...";

    //checking if length is bigger than 8 and not bad or not and getting the right messages
    if(checkLength(stringPassword) === true)
    {
        stringOutput = stringMessage1 + "\n" + stringMessage3;
        if(checkBad(stringPassword, arrayBadPasswords) === false)
        {
            stringOutput = stringMessage1 + "\n" + stringMessage3; 
        }
        else
    ...
    //checking if the password is bad and getting the messages accordingly
    ...
    $("output").value = "The password: " + stringPassword + "\n" + stringOutput;
    $("password").value = "";
...
{
    //returning the right password lenght else is password is not long enough
...
function checkBad(password, array) 
{
//returning only good passwords not ones ones in the array else is bad password
    if(array.includes(password) === true)
    {
        return true; 
    }
    else
